...
121 KLINGX = 1          : REM Constant for KLINGONS array index for X coordinate
122 KLINGY = 2          : REM Constant for KLINGONS array index for Y coordinate  
123 KLINGSTR = 3        : REM Constant for KLINGONS array index for strength
...
313 TORPEDOX = 0        : REM Integer X-coordinate for torpedo track
314 TORPEDOY = 0        : REM Integer Y-coordinate for torpedo track
...
331 WARPTIME = 0        : REM Time elapsed in warp
332 WARP = 0            : REM Warp factor input
333 ISOUTSIDE = 0       : REM Boundary crossing flag (1=outside of galaxy, 0=inside)
...
350 FOUND = 0           : REM Flag for object detection at coordinates (1=found, 0=not found)
351 TARGETX = 0         : REM X-coordinate of target for navigation calculations
352 TARGETY = 0         : REM Y-coordinate of target for navigation calculations 
353 STARTX = 0          : REM X-coordinate of starting point for navigation calculations
354 STARTY = 0          : REM Y-coordinate of starting point for navigation calculations
...
498 REM Function to calculate distance between Enterprise and a Klingon ship
...
1099 REM Ensure at least one starbase exists in the galaxy
...
2361 INPUT WARP
2362 PRINT
2363 IF DEVICE(WARPENGINES)<0 AND WARP>.2 THEN GOTO 2470
2380 IF WARP>0 AND WARP<=8 THEN GOTO 2490
2390 IF WARP=0 THEN GOTO 1990 : REM Main Loop
2420 PRINT"   CHIEF ENGINEER SCOTT: 'THE ENGINES WON'T TAKE";
2430 PRINT" WARP";WARP;"!'"
...
2510 PRINT"               FOR MANEUVERING AT WARP";WARP;"!'"
...
2702 D1=0
2703 D6=WARP          : REM Repair factor based on warp speed
2704 IF WARP>=1 THEN D6=1  : REM Cap repair factor at 1.0 for high warp

2770 FOR I=1 TO 8
2771     IF DEVICE(I)>=0 THEN GOTO 2880
2790     DEVICE(I)=DEVICE(I)+D6  : REM Repair damaged devices by D6 amount
...
3350     PRINT"SECTOR";SECTX;",";SECTY;"DUE TO BAD NAVIGATION"
3351     GOTO 3370
3360 NEXT I
...
3375 WARPTIME=1
3430 IF WARP<1 THEN WARPTIME=.1*INT(10*WARP)
3450 DATE=DATE+WARPTIME
...
3620 ISOUTSIDE = 0      : REM Reset boundary crossing flag
3621 IF QUADX<1 THEN ISOUTSIDE=1:QUADX=1:SECTX=1
3670 IF QUADX>8 THEN ISOUTSIDE=1:QUADX=8:SECTX=8
3710 IF QUADY<1 THEN ISOUTSIDE=1:QUADY=1:SECTY=1
3750 IF QUADY>8 THEN ISOUTSIDE=1:QUADY=8:SECTY=8
3790 IF ISOUTSIDE=0 THEN GOTO 3860
...
4920 X=X+X1
4921 Y=Y+X2
4922 TORPEDOX=INT(X+.5)
4923 TORPEDOY=INT(Y+.5)
4960 IF TORPEDOX<1 OR TORPEDOX>8 OR TORPEDOY<1 OR TORPEDOY>8 THEN GOTO 5490
5000 PRINT"               ";TORPEDOX;",";TORPEDOY
5001 A$="   "
5002 TEMPX=X
5003 TEMPY=Y
5004 GOSUB 8830 : REM Check for object in map
5050 IF FOUND<>0 THEN GOTO 4920
5060 A$="+K+"
5061 TEMPX=X
5062 TEMPY=Y
5063 GOSUB 8830 : REM Check for object in map
5064 IF FOUND=0 THEN GOTO 5210
5110 PRINT
...
5150 FOR I=1 TO 3
5151     IF TORPEDOX=KLINGONS(I,KLINGX) AND TORPEDOY=KLINGONS(I,KLINGY) THEN GOTO 5190
5180 NEXT I
5181 I=3
...
5214 IF FOUND=0 THEN GOTO 5280
5260 PRINT"STAR AT";TORPEDOX;",";TORPEDOY;"ABSORBED TORPEDO ENERGY."
...
5284 IF FOUND=0 THEN GOTO 4760 : REM Use ">B<" to match placed base symbol
...
6494 IF FOUND=1 THEN GOTO 6580
6540     NEXT J
6541 NEXT I
6542 ISDOCKED=0
6543 GOTO 6650
...
8110     TARGETX=KLINGONS(I,KLINGX)     : REM Use TARGETX instead of WARP
8111     TARGETY=KLINGONS(I,KLINGY)     : REM Use TARGETY instead of X
8120     STARTX=SECTX                   : REM Use STARTX instead of C1
8121     STARTY=SECTY                   : REM Use STARTY instead of A
8122     GOTO 8220
...
8170     INPUT"PLEASE ENTER INITIAL COORDINATES (X,Y)";STARTX,STARTY
8200     INPUT"FINAL COORDINATES (X,Y)";TARGETX,TARGETY
8220     DELTAY=TARGETY-STARTY         : REM Y distance between points
8221     DELTAX=STARTX-TARGETX         : REM X distance between points
8222     IF DELTAY<0 THEN GOTO 8350
8250     IF DELTAX<0 THEN GOTO 8410
8260     IF DELTAY>0 THEN GOTO 8280
8270     IF DELTAX=0 THEN C1=5:GOTO 8290
8280     C1=1
8290     IF ABS(DELTAX)<=ABS(DELTAY)THEN GOTO 8330
8310     PRINT"DIRECTION =";C1+(((ABS(DELTAX)-ABS(DELTAY))+ABS(DELTAX))/ABS(DELTAX))
8311     GOTO 8460
8330     PRINT"DIRECTION =";C1+(ABS(DELTAX)/ABS(DELTAY))
8331     GOTO 8460
8350     IF DELTAX>0 THEN C1=3:GOTO 8420
8360     IF DELTAY<>0 THEN C1=5:GOTO 8290
8410     C1=7
8420     IF ABS(DELTAX)>=ABS(DELTAY)THEN GOTO 8450
8430     PRINT"DIRECTION =";C1+(((ABS(DELTAY)-ABS(DELTAX))+ABS(DELTAY))/ABS(DELTAY))
8431     GOTO 8460
8450     PRINT"DIRECTION =";C1+(ABS(DELTAY)/ABS(DELTAX))
8460     PRINT"DISTANCE =";SQR(DELTAY^2+DELTAX^2)
8461     REM Alternate method for distance calculation having <12% error
8462     REM    MAX = maximum of ABS(DELTAX), ABS(DELTAY)
8463     REM    MIN = minimum of ABS(DELTAX), ABS(DELTAY)
8464     REM    DISTANCE = MAX + (MIN/2)
8480 NEXT I
...
8500 IF BASECNT<>0 THEN PRINT"FROM ENTERPRISE TO STARBASE:":TARGETX=BASEX:TARGETY=BASEY:STARTX=SECTX:STARTY=SECTY:GOTO 8220
...
8596 IF FOUND=0 THEN GOTO 8590
8600 RETURN
...
8829 REM Check for object at specific coordinates - Returns FOUND=1 if matching object found
