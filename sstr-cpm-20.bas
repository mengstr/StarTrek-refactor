...
280 REM Direction indices for DIRECTION array
281 XDIR = 1          : REM X-coordinate direction vector
282 YDIR = 2          : REM Y-coordinate direction vector
...
330 DIM GALAXY(8,8)      : REM Array storing info about each quadrant
331 DIM DIRECTION(9,2)   : REM Direction vectors for navigation
332 DIM KLINGONS(3,3)    : REM Array storing Klingon data (x,y,strength)
333 DIM N(3)             : REM Temporary array for scanning
334 DIM SEEN(8,8)        : REM Array of visited quadrants
335 DIM DEVICE(8)        : REM Ship devices status array

370 INITDATE=INT(RND(1)*20+20)*100 : REM Initial stardate (2000-4000)
371 DATE=INITDATE                   : REM Current stardate
372 TIMELIMIT=25+INT(RND(1)*10)    : REM Mission time limit
373 ISDOCKED=0                      : REM Flag for docked status
374 INITENERGY=3000                 : REM Initial energy level
375 ENERGY=INITENERGY               : REM Current energy level

440 INITTORP=10                     : REM Initial torpedo count
441 TORPEDOES=INITTORP              : REM Current torpedo count
442 MAXKLINGONSTR=200               : REM Maximum Klingon strength
443 SHIELD=0                        : REM Shield energy level
444 TOTALBASES=0                    : REM Total starbases in galaxy
445 TOTALKLINGONS=0                 : REM Total Klingons in galaxy
446 X$=""                           : REM Temporary string for plurals
447 X0$=" IS "                      : REM Temporary verb string
470 DEF FND(D)=SQR((KLINGONS(I,KLINGX)-SECTX)^2+(KLINGONS(I,KLINGY)-SECTY)^2) : REM Distance function

490 QUADX=INT(RND(1)*8+1)           : REM Starting quadrant X coordinate
491 QUADY=INT(RND(1)*8+1)           : REM Starting quadrant Y coordinate
492 SECTX=INT(RND(1)*8+1)           : REM Starting sector X coordinate
493 SECTY=INT(RND(1)*8+1)           : REM Starting sector Y coordinate

530 FOR I=1 TO 9
531     DIRECTION(I,XDIR)=0
532     DIRECTION(I,YDIR)=0
533 NEXT I
540 DIRECTION(3,XDIR)=-1
541 DIRECTION(2,XDIR)=-1
542 DIRECTION(4,XDIR)=-1
543 DIRECTION(4,YDIR)=-1
544 DIRECTION(5,YDIR)=-1
545 DIRECTION(6,YDIR)=-1
600 DIRECTION(1,YDIR)=1
601 DIRECTION(2,YDIR)=1
602 DIRECTION(6,XDIR)=1
603 DIRECTION(7,XDIR)=1
604 DIRECTION(8,XDIR)=1
605 DIRECTION(8,YDIR)=1
606 DIRECTION(9,YDIR)=1

670 FOR I=1 TO 8
671     DEVICE(I)=0
672 NEXT I

710 COMMANDS$="NAVSRSLRSPHATORSHEDAMCOMXXX"

720 REM Initialize variables
721 A$ = ""         : REM Temporary string for object symbol
722 NAME$ = ""      : REM Device or region name string
723 BASEX = 0       : REM X-coordinate of starbase
724 BASEY = 0       : REM Y-coordinate of starbase
725 TEMPX = 0       : REM Temporary X-coordinate
726 TEMPY = 0       : REM Temporary Y-coordinate
727 KLINGONCNT = 0  : REM Number of Klingons in current quadrant
728 BASECNT = 0     : REM Number of starbases in current quadrant 
729 STARCNT = 0     : REM Number of stars in current quadrant
730 BASENAMEONLY = 0: REM Flag for showing only base name
731 C$ = ""         : REM Ship condition (GREEN, YELLOW, RED)
732 Z3 = 0          : REM Flag for object detection at coordinates
733 Q4 = 0          : REM Previous quadrant X-coordinate
734 Q5 = 0          : REM Previous quadrant Y-coordinate
735 X1 = 0          : REM X-direction vector component
736 X2 = 0          : REM Y-direction vector component
737 X3 = 0          : REM Integer X-coordinate for torpedo track
738 Y3 = 0          : REM Integer Y-coordinate for torpedo track
739 X = 0           : REM Temporary X value for calculations
740 Y = 0           : REM Temporary Y value for calculations
741 C1 = 0          : REM Course input from player
742 S8 = 0          : REM Sector map index
743 J0 = 0          : REM Temporary value for tabulation
744 N = 0           : REM Energy units used in warp
745 D1 = 0          : REM Damage report flag
746 D6 = 0          : REM Warp factor for repair calculation
747 H = 0           : REM Hit points in combat
748 H1 = 0          : REM Phaser hit strength per Klingon
749 H8 = 0          : REM Flag for computer mode
750 I = 0           : REM Loop counter
751 J = 0           : REM Loop counter
752 L = 0           : REM Loop counter
753 R1 = 0          : REM Random or temporary value
754 R2 = 0          : REM Random coordinate Y
755 T8 = 0          : REM Time elapsed in warp
756 W1 = 0          : REM Warp factor input
757 X5 = 0          : REM Boundary crossing flag
758 REPAIRTIME = 0  : REM Time needed for repairs
759 SECTORMAP$ = "" : REM String containing sector map display
760 REGIONX = 0     : REM Current region X coordinate
761 REGIONY = 0     : REM Current region Y coordinate
762 REPAIRFACTOR = 0: REM Factor affecting repair time
763 O1$ = ""        : REM Line output string
