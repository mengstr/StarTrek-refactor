...
470 DEF FND(D)=SQR((KLINGONSTATUS(I,1)-ENTERPRISESECTORX)^2+(KLINGONSTATUS(I,2)-ENTERPRISESECTORY)^2)
...
1590 FOR I = 1 TO 3 
1591   KLINGONSTATUS(I,1) = 03 
1592   KLINGONSTATUS(I,2) = 03 
1593 NEXT I
1600 FOR I = 1 TO 3 
1601   KLINGONSTATUS(I,3) = 03 
1602 NEXT I
...
1992 REM ------------------------------------------------------------------------
1993 IF SHIELD+ENERGY>10 THEN IF ENERGY>10 OR DEVICESTATUS(7)=0 THEN GOTO 2060
2020 PRINT
...
2350 X$="8"
2351 IF DEVICESTATUS(1)<0 THEN X$="0.2"
2360 PRINT "WARP FACTOR (0-";X$;")";
2361 INPUT W1
2362 IF DEVICESTATUS(1)<0 AND W1>.2 THEN GOTO 2470
...
2530 IF SHIELD < N-ENERGY OR DEVICESTATUS(7)<0 THEN GOTO 1990
...
2770 FOR I=1 TO 8
2771   IF DEVICESTATUS(I)>=0 THEN GOTO 2880
2790   DEVICESTATUS(I)=DEVICESTATUS(I)+D6
2791   IF DEVICESTATUS(I)>-.1 AND DEVICESTATUS(I)<0 THEN DEVICESTATUS(I)=-.1 : GOTO 2880
2800   IF DEVICESTATUS(I)<0 THEN GOTO 2880
2810   IF D1<>1 THEN D1=1 : PRINT "DAMAGE CONTROL REPORT:  ";
2840   PRINT TAB(8);
2841   R1=I
2842   GOSUB 8790 : REM Get device name for message
...
2930 DEVICESTATUS(R1)=DEVICESTATUS(R1)-(RND(1)*5+1)
2931 PRINT "DAMAGE CONTROL REPORT:  ";
2960 GOSUB 8790 : REM Get device name for message
2961 PRINT G2$;" DAMAGED"
2962 PRINT
2963 GOTO 3070

3000 DEVICESTATUS(R1)=DEVICESTATUS(R1)+RND(1)*3+1
3001 PRINT "DAMAGE CONTROL REPORT:  ";
3030 GOSUB 8790 : REM Get device name for message
...
3991 REM ------------------------------------------------------------------------
4000 IF DEVICESTATUS(3)<0 THEN PRINT "LONG RANGE SENSORS ARE INOPERABLE" : GOTO 1990
...
4251 REM ------------------------------------------------------------------------
4260 IF DEVICESTATUS(4)<0 THEN PRINT "PHASERS INOPERATIVE" : GOTO 1990
...
4330 IF DEVICESTATUS(8)<0 THEN PRINT "COMPUTER FAILURE HAMPERS ACCURACY"
...
4411 IF DEVICESTATUS(7)<0 THEN X=X*RND(1)
...
4691 REM ------------------------------------------------------------------------
4700 IF TORPEDOCNT<=0 THEN PRINT "ALL PHOTON TORPEDOES EXPENDED" : GOTO 1990
4730 IF DEVICESTATUS(5)<0 THEN PRINT "PHOTON TUBES ARE NOT OPERATIONAL" : GOTO 1990
...
5521 REM ------------------------------------------------------------------------
5530 IF DEVICESTATUS(7)<0 THEN PRINT "SHIELD CONTROL INOPERABLE" : GOTO 1990
5560 PRINT "ENERGY AVAILABLE =";ENERGY+SHIELD;
5561 INPUT"NUMBER OF UNITS TO SHIELDS";X
5580 IF X<0 OR SHIELD=X THEN PRINT "<SHIELDS UNCHANGED>" : GOTO 1990
...
5681 REM ------------------------------------------------------------------------
5690 IF DEVICESTATUS(6)>=0 THEN GOTO 5910
5700 PRINT "DAMAGE CONTROL REPORT NOT AVAILABLE"
5701 IF ISDOCKED=0 THEN GOTO 1990
5720 D3 = 0
5721 FOR I = 1 TO 8
5722   IF DEVICESTATUS(I) < 0 THEN D3 = D3 + .1
5760 NEXT I
...
5870 FOR I=1 TO 8
5871   IF DEVICESTATUS(I)<0 THEN DEVICESTATUS(I)=0
5890 NEXT I
5891 STARDATE=STARDATE+D3+.1
5910 PRINT
5911 PRINT "DEVICE             STATE OF REPAIR"
5912 FOR R1 = 1 TO 8
5920   GOSUB 8790 : REM Get device name for message 
5921   PRINT G2$;LEFT$(Z$,25-LEN(G2$));INT(DEVICESTATUS(R1)*100)*.01
5950 NEXT R1
5951 PRINT
...
6141   DEVICESTATUS(R1)=DEVICESTATUS(R1)-H/SHIELD-.5*RND(1)
6142   GOSUB 8790 : REM Get device name for message
...
6720 IF DEVICESTATUS(2)>=0 THEN GOTO 6770
6730 PRINT
...
7281 REM ------------------------------------------------------------------------
7290 IF DEVICESTATUS(8)<0 THEN PRINT "COMPUTER DISABLED" : GOTO 1990
