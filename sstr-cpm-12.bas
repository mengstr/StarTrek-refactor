270 Z$="                         "
330 DIM GALAXY(8,8),C(9,2),K(3,3),N(3),SEEN(8,8),D(8)
...
1150 IF GALAXY(Q1,Q2)<200 THEN GALAXY(Q1,Q2)=GALAXY(Q1,Q2)+100:TOTALKLINGONS=TOTALKLINGONS+1
1160 TOTALBASES=1
1161 GALAXY(Q1,Q2)=GALAXY(Q1,Q2)+10
1162 Q1=INT(RND(1)*8+1)
1163 Q2=INT(RND(1)*8+1)

1200 K7=TOTALKLINGONS
...
1500 PRINT
1501 KLINGONCNT=INT(GALAXY(Q1,Q2)*.01)
1502 BASECNT=INT(GALAXY(Q1,Q2)*.1)-10*KLINGONCNT
1540 STARCNT=GALAXY(Q1,Q2)-100*KLINGONCNT-10*BASECNT
...
1781     K(I,1)=R1
1782     K(I,2)=R2
1783     K(I,3)=MAXKLINGONSTR*(.5+RND(1))
1784 NEXT I
...
4120         IF I>0 AND I<9 AND J>0 AND J<9 THEN N(J-Q2+2)=GALAXY(I,J):SEEN(I,J)=GALAXY(I,J)
4180     NEXT J
4181     FOR L=1 TO 3
4182         PRINT"| ";
...
4585     GOSUB 8670
4650     K(I,3)=0
4651     GALAXY(Q1,Q2)=GALAXY(Q1,Q2)-100
4652     SEEN(Q1,Q2)=GALAXY(Q1,Q2)
...
5432 A$="   "
5433 GOSUB 8670
5470 GALAXY(Q1,Q2)=KLINGONCNT*100+BASECNT*10+STARCNT
5471 SEEN(Q1,Q2)=GALAXY(Q1,Q2)
5472 GOSUB 6000
...
6381 PRINT
6400 PRINT"YOUR EFFICIENCY RATING IS";1000*(K7/(DATE-INITDATE))^2
6401 GOTO 6290
